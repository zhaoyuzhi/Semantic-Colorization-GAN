...
def trainer_noGAN(opt):
    ...
    # Define the dataset
    train_dataset = dataset.NIR_Colorization_with_Sal_dataset(opt, 'train')
    val_dataset = dataset.NIR_Colorization_dataset(opt, 'val')
    ...
    for epoch in range(opt.epochs):
        for i, (true_L, true_RGB, true_sal) in enumerate(train_loader):
            ...
            print("\r[Epoch %d/%d] [Batch %d/%d] [Pixel-level Loss: %.4f] [Attention Loss: %.4f] Time_left: %s" %
                ((epoch + 1), opt.epochs, i, len(train_loader), loss_L1.item(), loss_attn.item(), time_left))
    ...
    for epoch in range(opt.epochs):
        for i, (true_L, true_RGB, true_sal) in enumerate(train_loader):
            ...
            print("\r[Epoch %d/%d] [Batch %d/%d] [Pixel-level Loss: %.4f] [Attention Loss: %.4f] [Perceptual Loss: %.4f] [D Loss: %.4f] [G Loss: %.4f] Time_left: %s" %
                ((epoch + 1), opt.epochs, i, len(train_loader), loss_L1.item(), loss_attn.item(), loss_percep.item(), loss_D.item(), loss_GAN.item(), time_left))
