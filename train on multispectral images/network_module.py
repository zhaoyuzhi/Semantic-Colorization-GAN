...
#              Fusion Block
# ----------------------------------------
class FusionResidualUpBlock(nn.Module):
    def __init__(self, in_channels, latent_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', sn = False):
        super(FusionResidualUpBlock, self).__init__()
        self.path1 = TransposeConv2dLayer(in_channels = in_channels, out_channels = in_channels, kernel_size = kernel_size, stride = stride, padding = padding, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, sn = sn)
        self.path2 = nn.Sequential(
            ResConv2dLayer(in_channels = in_channels, latent_channels = latent_channels, kernel_size = kernel_size, stride = stride, padding = padding, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, sn = sn),
            TransposeConv2dLayer(in_channels = in_channels, out_channels = in_channels, kernel_size = kernel_size, stride = stride, padding = padding, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, sn = sn)
        )
    ...
    def forward(self, x):
        path0 = F.interpolate(x, scale_factor = 2, mode = 'nearest')
        path1 = self.path1(x)
        path2 = self.path2(x)
        path3 = self.path3(x)
...
class FusionResidualDownBlock(nn.Module):
    def __init__(self, in_channels, latent_channels, kernel_size = 3, stride = 1, padding = 1, dilation = 1, pad_type = 'zero', activation = 'lrelu', norm = 'none', sn = False):
        super(FusionResidualDownBlock, self).__init__()
        self.path1 = Conv2dLayer(in_channels = in_channels, out_channels = in_channels, kernel_size = 3, stride = 2, padding = 1, dilation = 1, pad_type = pad_type, activation = activation, norm = norm, sn = sn)
        self.path2 = nn.Sequential(
            ResConv2dLayer(in_channels = in_channels, latent_channels = latent_channels, kernel_size = kernel_size, stride = stride, padding = padding, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, sn = sn),
            Conv2dLayer(in_channels = in_channels, out_channels = in_channels, kernel_size = 3, stride = 2, padding = 1, dilation = 1, pad_type = pad_type, activation = activation, norm = norm, sn = sn)
        )
        self.path3 = nn.Sequential(
            ResConv2dLayer(in_channels = in_channels, latent_channels = latent_channels, kernel_size = kernel_size, stride = stride, padding = padding, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, sn = sn),
            ResConv2dLayer(in_channels = in_channels, latent_channels = latent_channels, kernel_size = kernel_size, stride = stride, padding = padding, dilation = dilation, pad_type = pad_type, activation = activation, norm = norm, sn = sn),
    ...
    def forward(self, x):
        path0 = F.interpolate(x, scale_factor = 0.5, mode = 'nearest')
        path1 = self.path1(x)
        path2 = self.path2(x)
        path3 = self.path3(x)
        out = path0 + path1 + path2 + path3
...
if __name__ == "__main__":
    ...
    net = FusionResidualDownBlock(256, 64, 3, 1, 1).cuda()
    print(net)
